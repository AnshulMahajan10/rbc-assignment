...
const getSQLRecords = async () => {
  try {
    let pool = await sql.connect(dbConfig.config);
    let records = pool.request().query("select * from records");
    return records;
  }
  catch (error) {
    console.log(error);
